...
$where_search_open = '';
if ($search_po !== '') {
    $s = mysqli_real_escape_string($koneksi, $search_po);
    $where_search_open = "AND (p.no_po LIKE '%$s%' OR r.no_request LIKE '%$s%' OR s.nama_supplier LIKE '%$s%')";
}
...
if ($search_po !== '') {
    $s = mysqli_real_escape_string($koneksi, $search_po);
    $where_search_close = "AND (p.no_po LIKE '%$s%' OR r.no_request LIKE '%$s%' OR s.nama_supplier LIKE '%$s%')";
}
...
if ($id_po_filter) {
    ...
    if ($po_detail) {
        ...
        $q_realisasi       = mysqli_query($koneksi,
            "SELECT SUM(pb.qty * pb.harga) AS total_nota
             FROM pembelian pb
             JOIN tr_request_detail d ON pb.id_request_detail = d.id_detail
...
if ($detail_items) {
    while ($d = mysqli_fetch_assoc($detail_items)) {
        ...
        if ($status === 'TERBELI')                 $item_dibeli++;
        elseif ($status === 'MENUNGGU VERIFIKASI') $item_verif++;
        else                                        $item_pending++;
        $rows_item[] = $d;
    }
}
...
.po-item-sup  { font-size: .75rem; font-weight: 600; color: var(--text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 240px; }
...
.approval-item strong { display: block; font-size: .63rem; font-weight: 800; text-transform: uppercase; letter-spacing: .4px; color: var(--slate); margin-bottom: 2px; }
.approval-item .appr-name { font-size: .76rem; font-weight: 700; color: var(--text); }
.approval-item .appr-date { font-size: .63rem; color: var(--slate); margin-top: 1px; }
.approval-item .appr-empty { color: #ccc; font-style: italic; font-size: .72rem; }
...
.info-val.text-money { font-family: 'JetBrains Mono', monospace; }
...
.btn-buka-pdf:hover { background: #15803d; color: #fff; }
...
<div class="main">
    ...
    <div class="split-wrap">
        ...
        <div class="sidebar-card">
            ...
            <div class="sidebar-search">
                <form method="GET" action="">
                    ...
                    <input type="text"
                           name="cari"
                           placeholder="Cari no. PO, no. request, supplier…"
            ...
            <?php else:
                while ($po_row = mysqli_fetch_assoc($result_aktif)):
                    ...
                    // Cek apakah ada nota untuk PO ini
                    $id_req_row = (int)$po_row['id_request'];
                    $cek_nota   = mysqli_query($koneksi,
                        "SELECT 1 FROM pembelian pb
                         JOIN tr_request_detail d ON pb.id_request_detail = d.id_detail
                         WHERE d.id_request = $id_req_row
                         LIMIT 1");
                    $ada_nota   = mysqli_num_rows($cek_nota) > 0;
            ?>
                <a class="po-item <?= $cls ?>"
                   href="?tab=<?= $tab_aktif ?>&id_po=<?= (int)$po_row['id_po'] ?><?= $search_po ? '&cari=' . urlencode($search_po) : '' ?>">
                    ...
                    <div class="po-item-amt">
                        ...
                        <?php if ($ada_nota): ?>
                            <span class="badge bg-success bg-opacity-10 text-success border border-success ms-1" style="font-size:.6rem;padding:1px 5px;">NOTA</span>
                        <?php endif; ?>
                    </div>
        ...
        <div class="detail-head">
                ...
                <div style="flex:1; min-width:0;">
                    ...
                    <div class="approval-strip">
                        ...
                        <div class="approval-item">
                            <strong><i class="fas fa-user-check me-1"></i>Appr <?= $n ?></strong>
                            <?php if (!empty($po_detail["approve{$n}_by"])): ?>
                                <div class="appr-name"><?= htmlspecialchars(strtoupper($po_detail["approve{$n}_by"])) ?></div>
                                <div class="appr-date"><?= !empty($po_detail["approve{$n}_at"]) ? date('d/m/y H:i', strtotime($po_detail["approve{$n}_at"])) : '' ?></div>
                            <?php else: ?>
                                <div class="appr-empty">—</div>
                            <?php endif; ?>
                        </div>
                        <?php endforeach; ?>
                    </div>
            ...
            <!-- Info grid — FIXED: semua kolom dalam grid yang benar -->
