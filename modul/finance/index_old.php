...
/* Mobile: sidebar di atas detail */
@media(max-width:991px){
    .po-list{max-height:260px;}
}
</style>
</head>
<body>
            ...
            <div style="flex: 1;">
                ...
                <div class="mt-3 p-2" style="background: #f8f9fa; border-radius: 5px; border-left: 3px solid #2c3e50; font-size: 8.5pt;">
                    ...
                    <div class="d-flex gap-2 flex-wrap no-print">
                        <a href="../transaksi/cetak_pr.php?id=<?= $id_request_sel ?>" 
                ...
                <table class="item-table">
                    <thead>
                        <tr>
                            ...
                            <!-- AKSI DINONAKTIFKAN — uncomment jika finance butuh aksi
                    ...
                    <tr class="<?= $is_ban ? 'row-ban' : '' ?>">
                        ...
                        <td>
                            ...
                            <div style="font-size:.67rem;color:var(--slate);margin-top:1px;"><?= htmlspecialchars($d['kwalifikasi']) ?></div>
                            <?php endif; ?>
                        </td>
                        ...
                        <td style="text-align:center;">
                            <?php if ($is_dibeli): ?>
                                ...
                                <div style="font-size:.65rem;color:var(--slate);margin-top:3px;">
                        ...
                        <td>
                            <?php if ($is_dibeli): ?>
                                <div style="font-weight:700;color:var(--teal-dk);font-size:.75rem;"><?= htmlspecialchars($d['dibeli_oleh'] ?? '-') ?></div>
                                <div style="font-size:.65rem;color:var(--slate);"><?= !empty($d['tgl_dibeli']) ? date('d/m/Y', strtotime($d['tgl_dibeli'])) : '' ?></div>
                            <?php else: ?>
                                <span style="color:var(--slate);">—</span>
                            <?php endif; ?>
                        </td>

                        <!-- ══ AKSI DINONAKTIFKAN UNTUK FINANCE ══
                             Uncomment blok di bawah jika suatu saat
                             finance perlu melakukan aksi tandai beli/pasang
                        <td style="text-align:center;">
                            <div class="d-flex flex-column gap-1">
                            <?php /*if (!$is_dibeli): */?>
                                <form method="POST" onsubmit="return konfirmBeli(event,'<?php /*= htmlspecialchars(strtoupper($nama),ENT_QUOTES) */?>',<?php /*= $id_po_filter */?>)">
                                    <input type="hidden" name="id_detail"  value="<?php /*= $d['id_detail'] */?>">
                                    <input type="hidden" name="id_po"      value="<?php /*= $id_po_filter */?>">
                                    <input type="hidden" name="id_request" value="<?php /*= $id_request_sel */?>">
                                    <input type="hidden" name="aksi"       value="beli">
                                    <button type="submit" class="btn btn-sm btn-success fw-bold" style="font-size:.7rem;">
                                        <i class="fas fa-shopping-bag me-1"></i>Tandai Dibeli
                                    </button>
                                </form>
                            <?php /*endif; */?>
                            <?php /*if ($is_ban && $is_dibeli && $status_pasang==='BELUM_TERPASANG'): */?>
                                <form method="POST" onsubmit="return konfirmPasang(event,'<?php /*= htmlspecialchars(strtoupper($nama),ENT_QUOTES) */?>','<?php /*= htmlspecialchars($unit,ENT_QUOTES) */?>',<?php /*= $id_po_filter */?>)">
                                    <input type="hidden" name="id_detail"  value="<?php /*= $d['id_detail'] */?>">
                                    <input type="hidden" name="id_po"      value="<?php /*= $id_po_filter */?>">
                                    <input type="hidden" name="id_request" value="<?php /*= $id_request_sel */?>">
                                    <input type="hidden" name="aksi"       value="pasang">
                                    <button type="submit" class="btn btn-sm btn-warning fw-bold" style="font-size:.7rem;">
...
/* ══ FUNGSI AKSI — DINONAKTIFKAN UNTUK FINANCE ══
   Uncomment jika suatu saat finance perlu aksi

function konfirmBeli(e, nama, idPo) {
    e.preventDefault();
    var form = e.target;
    if (confirm('Tandai "' + nama + '" sudah dibeli?')) {
        form.submit();
    }
    return false;
}

function konfirmPasang(e, nama, plat, idPo) {
    e.preventDefault();
    var form = e.target;
    if (confirm('Tandai ban "' + nama + '" pada kendaraan ' + plat + ' sudah terpasang?')) {
        form.submit();
    }
    return false;
}
