...
// ── QUERY PR yang butuh aksi dari manager ini ────────────────
// Mencakup kategori BESAR dan IT
// BUG FIX #1: tambahkan 'IT' di kategori_pr
// BUG FIX #2: perbaiki logika kondisi APPROVED 1
$sql = "SELECT * FROM tr_request
        WHERE kategori_pr IN ('BESAR', 'IT')
        AND status_request NOT IN ('BATAL','SELESAI')
        AND (
            -- Belum ada yang approve sama sekali → semua manager boleh approve sebagai M1
            (
                status_approval = 'MENUNGGU APPROVAL'
                AND (approve1_by IS NULL OR approve1_by = '')
            )
            OR
            -- Sudah APPROVED 1 → manager lain (bukan M1) boleh approve sebagai M2
            (
                status_approval = 'APPROVED 1'
                AND approve1_by != '$username_saya'
                AND (approve2_by IS NULL OR approve2_by = '')
            )
            OR
            -- Sudah APPROVED 2 → manager ke-3 yang ditunjuk boleh approve final
            (
...
<div class="container pb-5">
    ...
    <div class="row align-items-center mb-4 g-3">
        <div class="col-md-7">
            ...
            <p class="text-muted small mb-0">Menampilkan PR Barang Besar & IT yang menunggu tindakan persetujuan Anda.</p>
        </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <?php endwhile; else: ?>
                        <tr>
                            <!-- BUG FIX #3: colspan disesuaikan dengan jumlah kolom (8) -->
...
<script>
    // BUG FIX #4: path keep_alive.php disamakan konsisten
