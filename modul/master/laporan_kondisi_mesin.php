...
<head>
    ...
    <title>Laporan Kondisi Mesin - MCP System</title>
...
<nav class="navbar navbar-mcp mb-4 no-print">
    <div class="container-fluid px-3 px-sm-4">
        <span class="navbar-brand fw-bold text-white"><i class="fas fa-chart-bar me-2"></i> LAPORAN KONDISI MESIN</span>
        <div class="d-flex flex-wrap gap-1">
            <a href="../../index.php" class="btn btn-sm btn-danger"><i class="fas fa-rotate-left"></i> HOME</a>
            <a href="mesin.php" class="btn btn-sm btn-light fw-bold"><i class="fas fa-industry"></i> MESIN</a>
            <a href="kondisi_mesin.php" class="btn btn-sm btn-warning fw-bold"><i class="fas fa-heartbeat"></i> KONDISI</a>
        </div>
    </div>
</nav>
...
<div class="container-fluid px-3 px-sm-4">
    ...
    <div class="card mb-4 no-print">
        <div class="card-body">
            <form method="GET" class="row g-3 align-items-end">
                ...
                <div class="col-6 col-md-2">
                    <a href="print_laporan_kondisi_mesin.php?bulan=<?= $bulan_str ?>&tahun=<?= $tahun ?>" class="btn btn-success w-100" target="_blank">
    ...
    // Total Mesin Sedang Diservice (Aktif)
    ...
    // Total Selesai Service Bulan Ini
    ...
    <div class="row mb-2">
        <div class="col-6 col-md-3">
            <div class="stat-card total">
                ...
                <div class="stat-number"><?= $total_mesin ?></div>
                <div class="stat-label">Total Mesin Aktif</div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="stat-card aktif">
                ...
                <div class="stat-label">Sedang Diservice (Aktif Saat Ini)</div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="stat-card selesai">
                ...
                <div class="stat-label">Selesai Service Bulan Ini</div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="stat-card durasi">
                ...
                <div class="stat-label">Rata-rata Durasi Service</div>
            </div>
        </div>
    </div>
    ...
    <div class="row mb-4">
        <div class="col-12 col-md-6">
            <div class="card">
                <div class="card-header bg-light">
                    <h6 class="mb-0"><i class="fas fa-chart-pie me-2"></i>Distribusi Kondisi Mesin Saat Ini</h6>
                </div>
                <div class="card-body">
        ...
        <div class="col-12 col-md-6">
            <div class="card">
                <div class="card-header bg-light">
                    <h6 class="mb-0"><i class="fas fa-chart-bar me-2"></i>Durasi Service Terlama (Top 10)</h6>
                </div>
                <div class="card-body">
    ...
    <div class="card mt-4">
        <div class="card-header bg-light">
            <h6 class="mb-0"><i class="fas fa-history me-2"></i>Riwayat Service Dimulai Bulan <?= date('F Y', mktime(0, 0, 0, $bulan, 1, $tahun)) ?></h6>
        </div>
        <div class="card-body">
                ...
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        $stmt_riwayat = mysqli_prepare($koneksi, "
                            SELECT km.kondisi_mesin, km.start_date, km.end_date, 
                                   mm.id_mesin, mm.name
                        ...
                        mysqli_stmt_bind_result($stmt_riwayat, $r_kondisi, $r_start, $r_end, $r_id_mesin, $r_name);

                        $riwayat_grouped = [];
                        while (mysqli_stmt_fetch($stmt_riwayat)) {
                            $riwayat_grouped[$r_id_mesin]['id_mesin'] = $r_id_mesin;
                            $riwayat_grouped[$r_id_mesin]['name'] = $r_name;
                            $riwayat_grouped[$r_id_mesin]['episodes'][] = [
                                'kondisi' => $r_kondisi,
                                'start'   => $r_start,
                                'end'     => $r_end,
                            ];
                        ...
                        <tr>
                            <td colspan="5" class="text-center text-muted py-3">Tidak ada service yang dimulai pada bulan ini.</td>
                        </tr>
                        <?php else: foreach ($riwayat_grouped as $id_mesin => $data):
                            $episodes = $data['episodes'];
                            $terakhir = $episodes[0];
                            $aktif_terakhir = is_null($terakhir['end']);
                            $collapse_id = 'riwayat_' . preg_replace('/[^A-Za-z0-9]/', '', $id_mesin);
                        ?>
                        <tr class="riwayat-toggle" style="cursor:pointer;" data-bs-toggle="collapse" data-bs-target="#<?= $collapse_id ?>">
                            ...
                            <td class="fw-bold text-primary"><?= htmlspecialchars($data['id_mesin']) ?></td>
                            ...
                            <td>
                                <?php if ($aktif_terakhir && $terakhir['kondisi'] == 'DISERVICE') { ?>
                                    <span class="badge bg-warning text-dark">AKTIF SERVICE</span>
                                <?php } elseif ($aktif_terakhir && $terakhir['kondisi'] == 'BAIK') { ?>
...
$q_mesin = mysqli_query($koneksi, "SELECT id FROM master_mesin WHERE active = 1");
$stmt_cek = mysqli_prepare($koneksi,
    "SELECT kondisi_mesin FROM kondisi_mesin WHERE id_mesin = ? AND end_date IS NULL ORDER BY start_date DESC LIMIT 1"
);
mysqli_stmt_bind_param($stmt_cek, "i", $id_mesin_loop);
...
// Data untuk grafik durasi terlama
$query_durasi = mysqli_query($koneksi, "
    SELECT mm.id_mesin, mm.name,
           DATEDIFF(COALESCE(km.end_date, NOW()), km.start_date) as durasi
    FROM kondisi_mesin km
    JOIN master_mesin mm ON km.id_mesin = mm.id
    WHERE km.start_date IS NOT NULL
...
while ($row = mysqli_fetch_assoc($query_durasi)) {
    $durasi_labels[] = $row['id_mesin'] . ' - ' . substr($row['name'], 0, 15) . (strlen($row['name']) > 15 ? '...' : '');
...
$(document).on('show.bs.collapse', function (e) {
    var toggle = $('[data-bs-target="#' + e.target.id + '"]');
    toggle.find('.toggle-icon').removeClass('fa-chevron-right').addClass('fa-chevron-down');
});
$(document).on('hide.bs.collapse', function (e) {
    var toggle = $('[data-bs-target="#' + e.target.id + '"]');
    toggle.find('.toggle-icon').removeClass('fa-chevron-down').addClass('fa-chevron-right');
});
