...
<nav class="navbar navbar-mcp mb-4">
    <div class="container-fluid px-3 px-sm-4">
        ...
        <div class="d-flex flex-wrap gap-1">
            ...
            <a href="kondisi_kendaraan.php" class="btn btn-sm btn-warning fw-bold"><i class="fas fa-clipboard-list"></i> ADD KONDISI MOBIL</a>
        </div>
    </div>
</nav>
                ...
                <table id="tabelMobil" class="table table-hover table-striped align-middle w-100">
                    ...
                    <tbody class="small">
                        ...
                        while($d = mysqli_fetch_array($query)){
                            // Ambil kondisi terbaru dari tabel kondisi_kendaraan
                            $query_kondisi = mysqli_query($koneksi, "SELECT kondisi FROM kondisi_kendaraan WHERE id_mobil = '{$d['id_mobil']}' ORDER BY created_at DESC LIMIT 1");
                            $kondisi = 'BAIK'; // Default
                            if (mysqli_num_rows($query_kondisi) > 0) {
                                $kondisi_data = mysqli_fetch_assoc($query_kondisi);
                                $kondisi = $kondisi_data['kondisi'];
                            }
                            
                            $badge = '';
                            if ($kondisi == 'BAIK') {
                                $badge = '<span class="badge badge-baik">BAIK</span>';
                            } else if ($kondisi == 'DISERVICE') {
                                $badge = '<span class="badge badge-diservice">DISERVICE</span>';
                            } else if ($kondisi == 'RUSAK RINGAN') {
                                $badge = '<span class="badge badge-rusak-ringan">RUSAK RINGAN</span>';
                            } else if ($kondisi == 'RUSAK BERAT') {
                                $badge = '<span class="badge badge-rusak-berat">RUSAK BERAT</span>';
                            }
                        ?>
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    ...
                                    <button class="btn btn-sm btn-outline-info" onclick="lihatKondisi(<?= $d['id_mobil'] ?>)" title="Lihat Riwayat Kondisi">
...
<!-- Modal Riwayat Kondisi -->
<div class="modal fade" id="modalRiwayat" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title"><i class="fas fa-history me-2"></i> Riwayat Kondisi Kendaraan</h5>
