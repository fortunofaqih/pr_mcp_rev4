...
// ── 3. HANDLE UPLOAD FILE PENAWARAN (OPSIONAL) ───────────────
$nama_file_penawaran = NULL;

if (isset($_FILES['file_penawaran']) && $_FILES['file_penawaran']['error'] === UPLOAD_ERR_OK) {
    $file     = $_FILES['file_penawaran'];
    $ekstensi = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
    $ukuran   = $file['size'];

    // Validasi format — hanya PDF
    if ($ekstensi !== 'pdf') {
        header("location:tambah_request_besar.php?pesan=gagal_format");
        exit;
    }

    // Validasi ukuran — maks 5MB
    if ($ukuran > 5 * 1024 * 1024) {
        header("location:tambah_request_besar.php?pesan=gagal_ukuran");
        exit;
    }
...
if (!is_dir($folder)) {
    ...
    $nama_file_penawaran = 'penawaran_' . $no_safe . '_' . date('Ymd_His') . '_' . bin2hex(random_bytes(4)) . '.pdf';

    if (!move_uploaded_file($file['tmp_name'], $folder . $nama_file_penawaran)) {
        // Gagal pindah file — lanjut simpan tanpa lampiran
        $nama_file_penawaran = NULL;
    }
}

// Siapkan value SQL untuk kolom file_penawaran
$file_penawaran_sql = $nama_file_penawaran
    ? "'" . mysqli_real_escape_string($koneksi, $nama_file_penawaran) . "'"
    : "NULL";
...
try {
    ...
    $sql_header = "INSERT INTO tr_request
        (no_request, tgl_request, nama_pemesan, nama_pembeli,
         ...
         keterangan, file_penawaran, created_by)
        VALUES
        ('$no_request','$tgl_request','$nama_pemesan','$nama_pembeli',
         'PENDING','BESAR','MENUNGGU APPROVAL',
         NULL, NULL, NULL, NULL,
         NULL, NULL, 0, NULL,
         '$keterangan', $file_penawaran_sql, '$created_by')";
    ...
    // Hapus file yang sudah terlanjur diupload jika transaksi DB gagal
    if ($nama_file_penawaran) {
        $path_file = 'C:/uploads_pr/penawaran/' . $nama_file_penawaran;
